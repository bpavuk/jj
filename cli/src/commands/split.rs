...
pub(crate) fn cmd_split(
    ...
    let second_commit = {
        ...
        let new_tree = if parallel {
            ...
            let target_conflict_labels = target.conflict_labels()?;
            MergedTree::merge(Merge::from_vec(vec![
                (
                    target_tree,
                    format!("split commit ({})", target.commit.conflict_label()),
                ),
                (target.selected_tree.clone(), target_conflict_labels.after),
                (target.parent_tree.clone(), target_conflict_labels.before),
            ]))
            .block_on()?
        } else {
            target_tree
